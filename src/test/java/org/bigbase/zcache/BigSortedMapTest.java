...
public class BigSortedMapTest {
  ...
  public static void setUp() {
    ...
    map = new BigSortedMapOld(10000000);
    totalLoaded = 1;
  ...
  public void testFirstKey() {
    ...
    res = map.put(firstKey, 0, firstKey.length, value, 0, value.length);
    assertEquals(true, res);
  ...
  private List<byte[]> delete(int num) {
    ...
    while (numDeleted < num) {
      ...
      byte [] key = ("KEY"+ i).getBytes();
      long len = map.get(key, 0, key.length, val, 0);
      if (len == Block.NOT_FOUND) {
        continue;
      } else {
  ...
  public void testPartialScannerStartRow() {
    ...
    BigSortedMapScannerOld scanner = map.getScanner(startKey, null);
    long start = System.currentTimeMillis();
    long count = 0;
    byte[] value = new byte[("VALUE"+ totalLoaded).length()];
    byte[] prev = null;
  ...
  public void testPartialScannerStopRow() {
    ...
    BigSortedMapScannerOld scanner = map.getScanner(null, stopKey);
    long start = System.currentTimeMillis();
    long count = 0;
    byte[] value = new byte[("VALUE"+ totalLoaded).length()];
    byte[] prev = null;
  ...
  public void testPartialScannerStartStopRow() {
    ...
    BigSortedMapScannerOld scanner = map.getScanner(startKey, stopKey);
    long start = System.currentTimeMillis();
    long count = 0;
    byte[] value = new byte[("VALUE"+ totalLoaded).length()];
    byte[] prev = null;
  ...
  public void testSequentialInsert() {
    ...
    BigSortedMapOld map = new BigSortedMapOld(1000);
    int counter = 0;
    while(true) {
      byte[] key = nextKeySeq(counter);
      byte[] value = nextValueSeq(counter);
      //System.out.println(new String(key));
      if(map.put(key, 0, key.length, value, 0, value.length)) {
        counter++;
      } else {
        counter--;
        break;
