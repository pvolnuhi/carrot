...
public class IndexBlock implements Comparable<IndexBlock>{
  ...
  public final static int DATA_BLOCK_STATIC_OVERHEAD = DATA_BLOCK_STATIC_PREFIX 
      + VERSION_SIZE + TYPE_SIZE ; // 28 bytes
  ...
  /*
   * First key of this index block (used for comparisons)
   * including version and type (DELETE = 0, PUT=1)
   */
  byte[] firstKey;
  long version;
  byte type;
  ...
  private final long version(long blockAddress) {
    return UnsafeAccess.toLong(blockAddress + DATA_BLOCK_STATIC_PREFIX + 
      KEY_SIZE_LENGTH + keyLength(blockAddress));
  }
  
  private final byte type(long blockAddress) {
    return UnsafeAccess.toByte(blockAddress + DATA_BLOCK_STATIC_PREFIX + KEY_SIZE_LENGTH 
      + keyLength(blockAddress) + VERSION_SIZE);
  }
 ...
 * @param version 
  ...
  private boolean putEmpty(byte[] key, int keyOffset, int keyLength, byte[] value, 
		  int valueOffset, int valueLength, long version, long expire) {
    DataBlock b = new DataBlock();
	  ...
	  UnsafeAccess.putByte(dataPtr + DATA_BLOCK_STATIC_PREFIX + KEY_SIZE_LENGTH + keyLength + 
	    VERSION_SIZE, (byte)Op.PUT.ordinal());
  ...
  void updateFirstKey(DataBlock block) {
    ...
    UnsafeAccess.putByte(indexPtr + DATA_BLOCK_STATIC_PREFIX + KEY_SIZE_LENGTH + keyLength + 
      VERSION_SIZE, (byte)type.ordinal());
  }
...
/** TODO: handle not found (new block)
   * Search position of a block, which can contain this key
   * @param key
   * @param keyOffset
   * @param keyLength
  ...
  /**
   * Delete operation. FOR NOW deleting first key in a data block does not 
   * affect its key in an index block
   * TODO: Is it safe?
   * @param key
   * @param keyOffset
   * @param keyLength
 * @param version 
  ...
  public OpResult delete(byte[] key, int keyOffset, int keyLength, long version) throws RetryOperationException {
  ...
  public long get(byte[] key, int keyOffset, int keyLength, byte[] valueBuf, 
      int valOffset, long version) throws RetryOperationException {
