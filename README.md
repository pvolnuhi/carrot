...
* Sparse bitmaps are very memory efficient when population counts (%% of set bits) is < 10% or > 90%. In this case it provides very decent compression. Sparse bitmaps length is limited only by available physical RAM (bit index is the signed 64-bit integer).
